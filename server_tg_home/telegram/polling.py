...
class TelegramPolling:
    ...
    def _register_handlers(self) -> None:
        ...
        async def command_help(message: Message) -> None:
            chat_id = await self._allowed_chat_id(message)
            if chat_id is None:
                return
            await self._handle_help(chat_id, _message_args(message))
        ...
        async def command_clip(message: Message) -> None:
            ...
            if chat_id is None:
                return
            await self._handle_clip(chat_id, _message_args(message))
        ...
        async def command_snapshot(message: Message) -> None:
            ...
            if chat_id is None:
                return
            await self._handle_snapshot(chat_id, _message_args(message))
        ...
        async def command_ac_on(message: Message) -> None:
            ...
            if chat_id is None:
                return
            await self._handle_ac_on(chat_id, _message_args(message))
        ...
        async def command_status(message: Message) -> None:
            ...
            if chat_id is None:
                return
            await self._handle_status(chat_id, _message_args(message))
        ...
        async def command_unknown(message: Message) -> None:
            ...
            if chat_id is None:
                return
            await self._reply(chat_id, "Unknown command. Use /help.")

    async def _allowed_chat_id(self, message: Message) -> int | None:
        chat_id = _message_chat_id(message)
        if chat_id is None:
            return None
        if not chat_is_allowed(self.settings.telegram, chat_id):
            await self._reply(chat_id, f"Chat id {chat_id} is not allowed.")
            return None
        return chat_id
    ...
    async def _handle_clip(self, chat_id: int, args: list[str]) -> None:
        if not args:
            await self._reply(chat_id, "Usage: /clip <camera> [seconds]")
            return
        camera_id = args[0]
        camera = self.settings.cameras.get(camera_id)
        if camera is None:
            await self._reply(chat_id, f"Unknown camera: {camera_id}")
            return
        try:
            duration = int(args[1]) if len(args) > 1 else camera.default_duration_sec
        except ValueError:
            await self._reply(chat_id, "Duration must be an integer number of seconds.")
            return
        ...
        await self._reply(chat_id, f"Clip job queued: {job_id}")

    async def _handle_snapshot(self, chat_id: int, args: list[str]) -> None:
        if not args:
            await self._reply(chat_id, "Usage: /snapshot <camera>")
            return
        camera_id = args[0]
        ...
        await self._reply(chat_id, f"Snapshot job queued: {job_id}")

    async def _handle_ac_on(self, chat_id: int, args: list[str]) -> None:
        if not args:
            await self._reply(chat_id, "Usage: /ac_on <climate.entity_id>")
            return
        entity_id = args[0]
        with new_session() as session:
        ...
        await self._reply(chat_id, f"Home Assistant job queued: {job_id}")

    async def _handle_status(self, chat_id: int, args: list[str]) -> None:
        with new_session() as session:
