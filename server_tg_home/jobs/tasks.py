...
@dramatiq.actor(queue_name=settings.app.queue_name, max_retries=0)
def process_job(job_id: str) -> None:
    init_db(settings.app.database_url)
    ensure_storage(settings)
    processor = JobProcessor(settings, retry_callback=enqueue_job)
    processor.process_job_id(job_id)


def enqueue_job(job_id: str, delay_ms: int = 0) -> None:
    if delay_ms > 0:
        process_job.send_with_options(args=(job_id,), delay=delay_ms)
    else:
