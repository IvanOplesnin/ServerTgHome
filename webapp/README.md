...
Администраторы видят кнопку запуска записи для выбранной камеры. Запрос создаёт
обычное задание `record_video_file` с настроенной для камеры длительностью,
а интерфейс отдельно показывает очередь, запись, сохранение и результат.
Запуск из Mini App не отправляет служебное сообщение в групповой чат; текущее
состояние ручных записей на SSD можно проверить командой `/recordings`.
Пользователям с ролью `viewer` управляющая кнопка и API записи недоступны.
