...
- Записывать и сразу отправлять видео по команде Telegram `/clip`.
- Запускать запись видео на диск/SSD по команде Telegram `/record`.
- Запускать запись из Mini App и показывать активные записи по `/recordings`.
- Показывать последние сохраненные видео по `/videos` и отправлять выбранное видео кнопкой.
...
- `/record entrance 120`: записать 120-секундное видео на диск без автоматической отправки.
- `/recordings`: показать выполняемые ручные записи на SSD и камеры в очереди.
- `/videos entrance`: показать последние 20 сохраненных видео камеры с кнопками отправки.
...
В Telegram-теме, которая привязана к камере через `telegram.camera_topics`, команды `/record` и `/videos` можно писать без `camera_id`:
...
`/record` сохраняет файл в `storage.path/<camera>/<date>/` и создает запись в таблице `videos`. После завершения worker отправляет текстовое уведомление с номером видео. `/videos` берет последние 20 существующих файлов из БД, показывает inline-кнопки и по нажатию отправляет выбранный файл в текущий чат/тему.

Admin-only команды: `/clip`, `/record`, `/recordings`, `/videos`, `/last`, `/snapshot`, `/arm`, `/disarm`, `/mute`, `/ac_on`, `/panel`, кнопки камеры в `door` панели, запуск записи из Mini App, кнопки отправки сохраненных видео и voice playback.
